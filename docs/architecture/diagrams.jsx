...
function Node({ x, y, w, h, n, density, accent, showCite, mode='card', dashed=false }) {
  ...
  // Vertical centering: when h is taller than the tight baseline, shift the
  // middle content (title / sub / divider / bullets) down by half the slack so
  // it sits centered between the (top-anchored) eyebrow and (bottom-anchored)
  // chips. Stores reserve 12px more for the elliptical caps.
  const tightH = isStore ? 210 : 220;
  const yShift = Math.max(0, (h - tightH) / 2);
  ...
  return (
    <g className="node" data-id={n.id}>
      ...
      {/* eyebrow row: phase code + path · store nodes push down to clear the cap ellipse */}
      <text x={x+16} y={y+24+(isStore?12:0)} className="node-eyebrow">P0{n.phase} · {n.kind.toUpperCase()}</text>
      <text x={x+w-16} y={y+24+(isStore?12:0)} className="node-path" textAnchor="end">{n.path}</text>

      {/* title */}
      <text x={x+16} y={y+62+yShift} className="node-title">{n.title}</text>

      {/* sub */}
      <text x={x+16} y={y+82+yShift} className="node-sub">{n.sub}</text>

      {/* divider */}
      <line x1={x+16} y1={y+96+yShift} x2={x+w-16} y2={y+96+yShift} stroke="var(--gold-ghost)"/>

      {/* bullets */}
      {showBullets && n.bullets.slice(0, bulletCount).map((b, i) => (
        <g key={i}>
          <circle cx={x+22} cy={y+118+yShift + i*20} r="1.4" fill="var(--gold)"/>
          <text x={x+32} y={y+122+yShift + i*20} className="node-data">{b}</text>
        </g>
      ))}
  ...
  return (
    <g>
      <rect x={x - w/2} y={y - h/2} width={w} height={h} rx="2"
            fill="var(--bg)" stroke="var(--gold-faint)" strokeWidth="0.7"/>
      {kindGlyph && (
...
function Edge({ from, to, fromSide='r', toSide='l', edge, density, bend='auto', labelDx=0, labelDy=0, pass='all' }) {
  const p1 = getPort(from, fromSide);
  const p2 = getPort(to, toSide);
  const { d } = routePath(p1, p2, { bend });
  const className = `edge ${edge.weight === 'thick' ? 'thick' : 'thin'} ${edge.faint ? 'faint' : ''}`;
  const arrowDir = { l:'r', r:'l', t:'b', b:'t' }[toSide];
  const showPath = pass === 'all' || pass === 'path';
  return (
    <g>
      {showPath && <path d={d} className={className}/>}
      {showPath && <Arrow x={p2.x} y={p2.y} dir={arrowDir}/>}
    </g>
  );
}
...
function VariantStreets({ density, showCite }) {
  ...
  const r = {
    ...
    // P3 (3 nodes) — +25px over original to give text a touch more breathing room
    pgcopy:    { x: cols[2], y: 140, w: colW, h: 235 },
    publish:   { x: cols[2], y: 455, w: colW, h: 235 },
    db:        { x: cols[2], y: 760, w: colW, h: 255 },
  ...
  // Many labels naturally sit at path midpoints, which on long edges puts them
  // on top of intermediate nodes. labelDx/labelDy nudge labels into clear
  // corridors (gaps between nodes or off to a side).
  const edges = [
    // cards→cluster: straight vertical through transport — push label DOWN
    // into the gap between transport (ends 680) and cluster (starts 780)
    { e: ef('cards','cluster'),     from: r.cards,     to: r.cluster,   fromSide:'b', toSide:'t', labelDy:160 },
    { e: ef('transport','cluster'), from: r.transport, to: r.cluster,   fromSide:'b', toSide:'t' },
    // cards→transport label sits in gap between cards and transport — push left a bit
    { e: ef('cards','transport'),   from: r.cards,     to: r.transport, fromSide:'b', toSide:'t', labelDx:-110 },
    // cards→trainer: corridor between col0 and col1 — push label up into clear space
    { e: ef('cards','trainer'),     from: r.cards,     to: r.trainer,   fromSide:'r', toSide:'l', labelDy:-110 },
    { e: ef('gameplay','trainer'),  from: r.gameplay,  to: r.trainer,   fromSide:'b', toSide:'t', labelDx:-90 },
    // cluster→pgcopy: long bend, label naturally falls on transport — push down
    // into gap below transport
    { e: ef('cluster','pgcopy'),    from: r.cluster,   to: r.pgcopy,    fromSide:'r', toSide:'l', bend:0.06, labelDy:160 },
    // trainer→pgcopy: bends late, label falls near pgcopy left edge — push down
    { e: ef('trainer','pgcopy'),    from: r.trainer,   to: r.pgcopy,    fromSide:'r', toSide:'l', bend:0.55, labelDy:200 },
    ...
    // db→api: corridor between col2 and col3 — push label up clear of search node
    { e: ef('db','api'),            from: r.db,        to: r.api,       fromSide:'r', toSide:'l', labelDy:-90 },
    // db→search: same corridor — push label down to its own row
    { e: ef('db','search'),         from: r.db,        to: r.search,    fromSide:'r', toSide:'l', labelDy:80 },
  ...
  return (
    <svg className="arch" viewBox={`0 0 ${W} ${H}`} xmlns="http://www.w3.org/2000/svg">
      ...
      {/* edge paths + arrows — behind nodes */}
      {edges.map((eg, i) => <Edge key={`p-${i}`} {...eg} edge={eg.e} density={density} pass="path"/>)}
      ...
      {/* edge labels — drawn LAST so they sit on top of nodes */}
      {edges.map((eg, i) => <Edge key={`l-${i}`} {...eg} edge={eg.e} density={density} pass="label"/>)}
