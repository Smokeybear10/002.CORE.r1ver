...
export function HeroSection() {
  ...
  useEffect(() => {
    const unsub = scrollStore.subscribe((s) => {
      ...
      p1.style.opacity = h < 0.3 ? "1" : String(Math.max(0, 1 - (h - 0.3) * 10))

      let p2op = 0
      if (h < 0.3) p2op = 0
      else if (h < 0.4) p2op = (h - 0.3) * 10
      else if (h < 0.6) p2op = 1
      else if (h < 0.7) p2op = 1 - (h - 0.6) * 10
      else p2op = 0
      p2.style.opacity = String(p2op)

      let p3op = 0
      if (h < 0.6) p3op = 0
      else if (h < 0.7) p3op = (h - 0.6) * 10
      else p3op = 1
      p3.style.opacity = String(p3op)

      if (equityRef.current) {
        if (h > 0.6) {
          const t = Math.min(1, (h - 0.6) / 0.4)
          const eased = t * (2 - t)
      ...
      <div className="hero-sticky">
        ...
        <div className="hero-content">
          <div className="hero-phase" ref={phase1Ref}>
            <div className="hero-eyebrow">— a solver you can watch think —</div>
